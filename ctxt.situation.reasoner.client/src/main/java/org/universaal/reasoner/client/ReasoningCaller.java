...
public class ReasoningCaller {
    ...
    private static final String OUTPUT_SITUATIONS = REASONER_CLIENT_NAMESPACE
	    + "currentSituations";
    private static final String OUTPUT_QUERIES = REASONER_CLIENT_NAMESPACE
	    + "currentQueries";
    private static final String OUTPUT_RULES = REASONER_CLIENT_NAMESPACE
	    + "currentRules";
     ...
     *         trouble.
     */
    public Situation addSituation(String subject, String predicate,
	    String object, boolean persistent) {
	return this.addSituation(ReasoningCaller.REASONER_CLIENT_NAMESPACE
		+ "Situation" + random.nextInt(9999999), subject, predicate,
		object, persistent);
    }
     ...
     *         trouble.
     */
    public Situation addSituation(String uri, String subject, String predicate,
	    String object, boolean persistent) {
	Situation situation = new Situation(uri, subject, predicate, object);
	situation.setPersistent(persistent);
	if (addObject(situation,
		new String[] { ReasoningService.PROP_SITUATIONS }))
	    return situation;
	return null;
    }
	...
	Query query = new Query(ReasoningCaller.REASONER_CLIENT_NAMESPACE
		+ "Query" + random.nextInt(9999999), fullQuery);
     ...
     */
    public Query addQuery(ContextEvent event, String queryString,
	    boolean persistent) {
	return this.addQuery(ReasoningCaller.REASONER_CLIENT_NAMESPACE
		+ "Query" + random.nextInt(9999999), event, queryString,
		persistent);
    }
     ...
     * @return The created Query. Can be null in case of any trouble.
     */
    public Query addQuery(String uri, ContextEvent event, String queryString,
	    boolean persistent) {
	...
	return addRule(ReasoningCaller.REASONER_CLIENT_NAMESPACE + "Rule"
		+ random.nextInt(9999999), situation, query, persistent);
    }
     ...
     *         trouble.
     */
    public Rule addRule(String uri, Situation situation, Query query,
	    boolean persistent) {
	...
	if (sr.getCallStatus() == CallStatus.succeeded) {
	    ...
	    LogUtils.logWarn(Activator.context, ReasoningCaller.class,
		    "addObject",
	...
	return getObjects(Situation.class, OUTPUT_SITUATIONS,
		new String[] { ReasoningService.PROP_SITUATIONS });
    }

    public List<Query> getQueries() {
	return getObjects(Query.class, OUTPUT_QUERIES,
		new String[] { ReasoningService.PROP_QUERIES });
    }

    public List<Rule> getRules() {
	return getObjects(Rule.class, OUTPUT_RULES,
		new String[] { ReasoningService.PROP_RULES });
    }
    ...
    private <M extends Persistent> List<M> getObjects(Class<M> requestClass,
	    String output_param, String[] ppPath) {
	ServiceRequest getRequest = new ServiceRequest(new ReasoningService(),
		null);
	...
	if (sr.getCallStatus() == CallStatus.succeeded) {
		...
		Activator.postInfo(ReasoningCaller.class, "getObjects",
			"Error during gathering objects from type "
				+ requestClass.getName());
		return null;
	    }
	} else {
	    Activator.postInfo(ReasoningCaller.class, "getObjects",
		    "callstatus is not succeeded for objects from type "
			    + requestClass.getName());
	    return null;
	}
    }
	...
	return removeObject(situation,
		new String[] { ReasoningService.PROP_SITUATIONS });
    }

    public boolean remove(Query query) {
	return removeObject(query,
		new String[] { ReasoningService.PROP_QUERIES });
    }
    ...
    private <M extends Persistent> boolean removeObject(M object,
	    String[] ppPath) {
	ServiceRequest removeRequest = new ServiceRequest(
		new ReasoningService(), null);
