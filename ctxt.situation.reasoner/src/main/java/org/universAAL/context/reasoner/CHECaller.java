...
public class CHECaller extends ServiceCaller {
	...
	public void executeQuery(String theQuery) {
		...
		if (!ser.isEmpty()) {
			...
			if (event.isWellFormed() && event.getSubjectTypeURI() != null) {
				...
				LogUtils.logError(Activator.mcontext, CHECaller.class, "executeQuery",
						new Object[] {
			...
			LogUtils.logWarn(Activator.mcontext, CHECaller.class, "executeQuery",
	...
	public String callDoSPARQL(String query) {
		...
		if (response.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				LogUtils.logInfo(Activator.mcontext, CHECaller.class, "callDoSPARQL",
	...
	private Object getReturnValue(List outputs, String expectedOutput) {
		...
		if (outputs == null)
			LogUtils.logInfo(Activator.mcontext, CHECaller.class, "getReturnValue",
