...
/**
 * Like a {@link org.universAAL.context.che.database.impl.SesameBackendCrd} that
...
public class SesameBackendCrdCnf extends SesameBackendWithConfidence {
	 ...
	 * Logger.
	 */
	private static final Log log = Hub.getLog(SesameBackendCrdCnf.class);
		...
		if (dataPath != null) {
			...
			try {
				myRepository = new SailRepository(
						new ForwardChainingRDFSInferencer(new CardinalityNativeStore(dataDir, indexes, encrypt)));
