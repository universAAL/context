...
public class ContextHistoryServices extends ContextHistoryService{
	...
	static final String SERVICE_GET_EVENTS_FROM_TIMESTAMP = CHE_NAMESPACE + "getEventsFromTimestamp";
	static final String SERVICE_GET_EVENTS_TO_TIMESTAMP = CHE_NAMESPACE + "getEventsToTimestamp";
	static final String SERVICE_GET_EVENTS_BETWEEN_TIMESTAMPS = CHE_NAMESPACE + "getEventsBetweenTimestamps";
	static final String SERVICE_DO_SPARQL_QUERY = CHE_NAMESPACE + "doSparqlQuery";
	static final String SERVICE_GET_EVENTS_BY_SPARQL = CHE_NAMESPACE + "getEventsBySparql";
	...
	static {
		...
		addRestriction((Restriction)
				ContextHistoryService.getClassRestrictionsOnProperty(ContextHistoryService.PROP_MANAGES).copy(),
				new String[] {ContextHistoryService.PROP_MANAGES},
				serverLevelRestrictions);
		addRestriction((Restriction)
				ContextHistoryService.getClassRestrictionsOnProperty(ContextHistoryService.PROP_PROCESSES).copy(),
				new String[] {PROP_PROCESSES},
				serverLevelRestrictions);
		addRestriction((Restriction)
				ContextHistoryService.getClassRestrictionsOnProperty(ContextHistoryService.PROP_RETURNS).copy(),
				new String[] {PROP_RETURNS},
				serverLevelRestrictions);
		addRestriction((Restriction)
				ContextHistoryService.getClassRestrictionsOnProperty(ContextHistoryService.PROP_TIMESTAMP_FROM).copy(),
				new String[] {PROP_TIMESTAMP_FROM},
				serverLevelRestrictions);
		addRestriction((Restriction)
				ContextHistoryService.getClassRestrictionsOnProperty(ContextHistoryService.PROP_TIMESTAMP_TO).copy(),
		...
		Restriction eventRestr = Restriction.getFixedValueRestriction(
				ContextHistoryService.PROP_MANAGES, eventInput.asVariableReference());
		
		Restriction queryr = Restriction.getFixedValueRestriction(
				ContextHistoryService.PROP_PROCESSES, queryInput.asVariableReference());
		
		Restriction tstFromRestr = Restriction.getFixedValueRestriction(
				ContextHistoryService.PROP_TIMESTAMP_FROM, tstFromInput.asVariableReference());
		
		Restriction tstToRestr = Restriction.getFixedValueRestriction(
				ContextHistoryService.PROP_TIMESTAMP_TO, tstToInput.asVariableReference());
		...
		PropertyPath managesPath = new PropertyPath(null, false, new String[] {
				ContextHistoryService.PROP_MANAGES});
		
		PropertyPath returnsPath = new PropertyPath(null, true, new String[] {
		...
		ContextHistoryServices getEventsFromTimestamp = new ContextHistoryServices(SERVICE_GET_EVENTS_FROM_TIMESTAMP);
		profiles[1] = getEventsFromTimestamp.getProfile();
		profiles[1].addInput(eventInput);
		getEventsFromTimestamp.addInstanceLevelRestriction(eventRestr, new String[] {ContextHistoryService.PROP_MANAGES});
		profiles[1].addInput(tstFromInput);
		getEventsFromTimestamp.addInstanceLevelRestriction(tstFromRestr, new String[] {ContextHistoryService.PROP_TIMESTAMP_FROM});
		profiles[1].addOutput(output);
		profiles[1].addSimpleOutputBinding(output,managesPath.getThePath());
		
		//GET_EVENTS_TO_TIMESTAMP
		ContextHistoryServices getEventsToTimestamp = new ContextHistoryServices(SERVICE_GET_EVENTS_TO_TIMESTAMP);
		profiles[2] = getEventsToTimestamp.getProfile();
		profiles[2].addInput(eventInput);
		getEventsToTimestamp.addInstanceLevelRestriction(eventRestr, new String[] {ContextHistoryService.PROP_MANAGES});
		profiles[2].addInput(tstToInput);
		getEventsToTimestamp.addInstanceLevelRestriction(tstToRestr, new String[] {ContextHistoryService.PROP_TIMESTAMP_TO});
		profiles[2].addOutput(output);
		profiles[2].addSimpleOutputBinding(output,managesPath.getThePath());
		
		//GET_EVENTS_BETWEEN_TIMESTAMPS
		ContextHistoryServices getEventsBetweenTimestamp = new ContextHistoryServices(SERVICE_GET_EVENTS_BETWEEN_TIMESTAMPS);
		profiles[3] = getEventsBetweenTimestamp.getProfile();
		profiles[3].addInput(eventInput);
		getEventsBetweenTimestamp.addInstanceLevelRestriction(eventRestr, new String[] {ContextHistoryService.PROP_MANAGES});
		profiles[3].addInput(tstFromInput);
		getEventsBetweenTimestamp.addInstanceLevelRestriction(tstFromRestr, new String[] {ContextHistoryService.PROP_TIMESTAMP_FROM});
		profiles[3].addInput(tstToInput);
		getEventsBetweenTimestamp.addInstanceLevelRestriction(tstToRestr, new String[] {ContextHistoryService.PROP_TIMESTAMP_TO});
		profiles[3].addOutput(output);
		profiles[3].addSimpleOutputBinding(output,managesPath.getThePath());
		
		//GET_EVENTS_BY_SPARQL_QUERY
		ContextHistoryServices doSPARQLforEvents = new ContextHistoryServices(SERVICE_GET_EVENTS_BY_SPARQL);
		profiles[4] = doSPARQLforEvents.getProfile();
		profiles[4].addInput(queryInput);
		doSPARQLforEvents.addInstanceLevelRestriction(queryr, new String[] {ContextHistoryService.PROP_PROCESSES});
