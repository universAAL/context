...
public class ArtifactIT extends IntegrationTest {
	...
	public void testProfilable() {
		caller = new DefaultServiceCaller(Activator.getModuleContext());
		LogUtils.logInfo(Activator.getModuleContext(), ArtifactIT.class, "testProfilable", new String[] { "-Test 1-" },
				null);
		...
		Assert.isTrue(str4.equals(user2.getURI()), "Should have retruned a AP, but not " + str4 + " with " + out2);
	...
	public void testProfile() {
		...
		AssistedPersonProfile prof2 = new AssistedPersonProfile(NAMESPACE + "apProf2");

		String str1 = getProfile(prof1);
		Assert.isTrue(str1.equals(NOTHING), "Should have returned nothing (1), but not " + str1);
		...
		Assert.isTrue(str5.equals(NOTHING), "Should have returned nothing (2), but not " + str5 + " with " + out4);
		// Clean
		removeProfile(prof2);
	}
	...
	public void testSubProfile() {
		caller = new DefaultServiceCaller(Activator.getModuleContext());
		LogUtils.logInfo(Activator.getModuleContext(), ArtifactIT.class, "testSubProfile", new String[] { "-Test 3-" },
				null);
	...
	private String removeProfilable(Resource profilable) {
		...
		MergedRestriction r1 = MergedRestriction.getFixedValueRestriction(ProfilingService.PROP_CONTROLS, profilable);
	...
	private String removeProfile(UserProfile profile) {
		ServiceRequest req = new ServiceRequest(new ProfilingService(null), null);
		MergedRestriction r1 = MergedRestriction.getFixedValueRestriction(Profilable.PROP_HAS_PROFILE, profile);
	...
	private String getProfile(UserProfile profile) {
		...
		req.addRequiredOutput(ARG_OUT, new String[] { ProfilingService.PROP_CONTROLS, Profilable.PROP_HAS_PROFILE });
		ServiceResponse resp = caller.call(req);
		if (resp.getCallStatus() == CallStatus.succeeded) {
			Object out = getReturnValue(resp.getOutputs(), ARG_OUT);
			if (out != null) {
	...
	private String removeSubProfile(SubProfile profile) {
		ServiceRequest req = new ServiceRequest(new ProfilingService(null), null);
		MergedRestriction r1 = MergedRestriction.getFixedValueRestriction(Profile.PROP_HAS_SUB_PROFILE, profile);
		req.getRequestedService().addInstanceLevelRestriction(r1, new String[] { ProfilingService.PROP_CONTROLS,
