...
public class QueryCreatorFrame extends JFrame {
	...
	uriText = new JTextField(Query.MY_URI
	...
	createQueryButton.addActionListener(new ActionListener() {
		...
		if (eventBox.getSelectedItem() == null) {
		    JOptionPane.showMessageDialog(self,
			    "No ContextEvent available up to now.");
		    return;
		}
		...
		if (event == null) {
		    JOptionPane.showMessageDialog(self,
			    "Need to select a ContextEvent first.");
		    return;
		}
		String search = searchStringArea.getText();
		if (search == null || search.equals("")) {
		    JOptionPane.showMessageDialog(self,
			    "No query-string is given!");
		    return;
		}
	...
	checkQuery.addActionListener(new ActionListener() {
		...
		ContextEvent checkEvent = Activator.cheCaller
		...
		if (!checkEvent.isWellFormed()
			|| checkEvent.getSubjectTypeURI() == null) {
		    JOptionPane
			    .showMessageDialog(
				    self,
				    "Invalid CONSTRUCT query associated to "
	...
	addButton.addActionListener(new ActionListener() {
		...
		if (!addQuery())
		    JOptionPane.showMessageDialog(self,
			    "Error during add of the new query.");
	...
	if (event != null) {
		...
		if (event.getRDFObject() instanceof ManagedIndividual)
		    objectLabel.setText("Object: "
			    + ((ManagedIndividual) event.getRDFObject())
				    .getURI());
		else
		    objectLabel.setText("Object: "
	...
	this.events.clear();
	ArrayList<ContextEvent> curEvents = caller.getRecorder()
		.getRecordedEvents();
	...
	String fullQuery = fullSearchText.getText();
	if (uri == null || uri.equals("") || fullQuery == null
		|| fullQuery.equals(""))
	    return false;
