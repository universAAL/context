...
public class SituationCreatorFrame extends JFrame {
	...
	c.gridy = 0;
	c.gridwidth = 2;
	JTextField uriText = new JTextField(Situation.MY_URI
		+ ReasoningCaller.random.nextInt(9999999));
	...
	c.gridy = 1;
	c.weightx = 1.0;
	subjectBox = new JComboBox(caller.getRecorder()
		.getAllTypeAndInstanceURIs().toArray(new String[0]));
	...
	c.gridy = 2;
	c.weightx = 1.0;
	predicateBox = new JComboBox(caller.getRecorder()
		.getRecordedPredicates().toArray(new String[0]));
	...
	c.gridy = 3;
	c.weightx = 1.0;
	objectBox = new JComboBox(caller.getRecorder()
		.getAllTypeAndInstanceURIs().toArray(new String[0]));
	...
	addButton.addActionListener(new ActionListener() {
		...
		String predicate = predicateCheck.isSelected() ? predicateBox
			.getSelectedItem().toString() : null;
		String object = predicateCheck.isSelected()
			&& objectCheck.isSelected() ? objectBox
			.getSelectedItem().toString() : null;
		caller.addSituation(subjectBox.getSelectedItem().toString(),
			predicate, object, saveCheck.isSelected());
