...
public class SCallee extends ServiceCallee {
	...
	this.addNewServiceProfiles( SCalleeProvidedService.getServiceProfiles(
		NAMESPACE_AALSPACE, ProfilingService.MY_URI,
		new String[] { ProfilingService.PROP_CONTROLS },
		AALSpace.MY_URI));
	this.addNewServiceProfiles(SCalleeProvidedService.getServiceProfiles(
		NAMESPACE_AALSPACEPROF, ProfilingService.MY_URI, new String[] {
			ProfilingService.PROP_CONTROLS,
			Profilable.PROP_HAS_PROFILE }, AALSpaceProfile.MY_URI));
	this.addNewServiceProfiles(SCalleeProvidedService.getServiceProfiles(
		NAMESPACE_AALSERVICE, ProfilingService.MY_URI,
		new String[] { ProfilingService.PROP_CONTROLS },
		AALService.MY_URI));
	this.addNewServiceProfiles(SCalleeProvidedService
		.getServiceProfiles(NAMESPACE_AALSERVICEPROF,
			ProfilingService.MY_URI, new String[] {
				ProfilingService.PROP_CONTROLS,
				Profilable.PROP_HAS_PROFILE },
			AALServiceProfile.MY_URI));
	this.addNewServiceProfiles(SCalleeProvidedService.getServiceProfiles(
		NAMESPACE_DEVICE, ProfilingService.MY_URI, new String[] {
			ProfilingService.PROP_CONTROLS,
			Profilable.PROP_HAS_PROFILE,
			AALSpaceProfile.PROP_INSTALLED_HARDWARE },
		Device.MY_URI));
	this.addNewServiceProfiles(SCalleeProvidedService.getServiceProfiles(
		NAMESPACE_ONT, ProfilingService.MY_URI, new String[] {
			ProfilingService.PROP_CONTROLS,
			Profilable.PROP_HAS_PROFILE,
...
//	this.addNewRegParams(SCalleeProvidedService.profiles);
	...
	if (operation.equals(NAMESPACE_AALSPACE
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_AALSPACE
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getAALSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_AALSPACE
	...
	if (operation.equals(NAMESPACE_AALSPACEPROF
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_AALSPACEPROF
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getAALSpaceProfile((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_AALSPACEPROF
		    + SCalleeProvidedService.OUT_GET_X, result));
	    return response;
	}
	...
	if (operation.equals(NAMESPACE_AALSERVICE
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_AALSERVICE
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getAALService((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_AALSERVICE
		    + SCalleeProvidedService.OUT_GET_X, result));
	    return response;
	}
	...
	if (operation.equals(NAMESPACE_AALSERVICEPROF
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_AALSERVICEPROF
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getAALServiceProf((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_AALSERVICEPROF
		    + SCalleeProvidedService.OUT_GET_X, result));
	    return response;
	}
	...
	if (operation.equals(NAMESPACE_DEVICE
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_DEVICE
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getDevice((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_DEVICE
		    + SCalleeProvidedService.OUT_GET_X, result));
	    return response;
	}
	...
	if (operation.equals(NAMESPACE_ONT
		+ SCalleeProvidedService.SRV_GET_X)) {
	    Object input = call.getInputValue(NAMESPACE_ONT
		    + SCalleeProvidedService.INP_GET_X);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getOnt((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(NAMESPACE_ONT
		    + SCalleeProvidedService.OUT_GET_X, result));
	    return response;
	}
	...
	if (operation.equals(SCalleeProvidedService.SRV_GET_SERVS)) {
	    ...
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_SERVS, result));
	    return response;
	}
	...
	if (operation.equals(SCalleeProvidedService.SRV_GET_SPACES)) {
	    ...
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_SPACES, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_HR_OF_SERV)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_HR_OF_SERV);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getHROfAALService((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_HR_OF_SERV, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_HW_OF_SERV)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_HW_OF_SERV);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getHWOfAALService((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_HW_OF_SERV, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_APP_OF_SERV)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_APP_OF_SERV);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getAppOfAALService((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_APP_OF_SERV, result));
	    return response;
	}

	if (operation.equals(SCalleeProvidedService.SRV_GET_SERVS_OF_SPACE)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_SERVS_OF_SPACE);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ArrayList result = Activator.scaller
		    .getServicesOfSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_SERVS_OF_SPACE, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_DEVS_OF_SPACE)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_DEVS_OF_SPACE);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ArrayList result = Activator.scaller
		    .getDevicesOfSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_DEVS_OF_SPACE, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_ONTS_OF_SPACE)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_ONTS_OF_SPACE);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ArrayList result = Activator.scaller
		    .getOntsOfSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_ONTS_OF_SPACE, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_OWNERS_OF_SPACE)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_OWNERS_OF_SPACE);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ArrayList result = Activator.scaller
		    .getOwnsOfSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_OWNERS_OF_SPACE, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_OWNERS_OF_SERV)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_OWNERS_OF_SERV);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ArrayList result = Activator.scaller
		    .getOwnsOfServ((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_OWNERS_OF_SERV, result));
	    return response;
	}
	
	if (operation.equals(SCalleeProvidedService.SRV_GET_PROF_OF_SERV)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_PROF_OF_SERV);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    ...
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    response.addOutput(new ProcessOutput(
		    SCalleeProvidedService.OUT_GET_PROF_OF_SERV,
		    (AALServiceProfile) result));
	    return response;
	}

	if (operation.equals(SCalleeProvidedService.SRV_GET_PROF_OF_SPACE)) {
	    Object input = call
		    .getInputValue(SCalleeProvidedService.INP_GET_PROF_OF_SPACE);
	    if (input == null) {
		return ERROR_INPUT;
	    }
	    Resource result = Activator.scaller
		    .getProfOfSpace((Resource) input);
	    ServiceResponse response = new ServiceResponse(CallStatus.succeeded);
	    if (result != null)//
		response.addOutput(new ProcessOutput(
			SCalleeProvidedService.OUT_GET_PROF_OF_SPACE,
			(AALSpaceProfile) result));
	    return response;
	}
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_SERV_TO_SPACE)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_SERV_TO_SPACE_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_SERV_TO_SPACE_WHAT);
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_DEV_TO_SPACE)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_DEV_TO_SPACE_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_DEV_TO_SPACE_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_ONT_TO_SPACE)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_ONT_TO_SPACE_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_ONT_TO_SPACE_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_OWNER_TO_SPACE)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_OWNER_TO_SPACE_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_OWNER_TO_SPACE_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_OWNER_TO_SERV)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_OWNER_TO_SERV_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_OWNER_TO_SERV_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_PROF_TO_SPACE)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_PROF_TO_SPACE_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_PROF_TO_SPACE_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	...
	if (operation.equals(SCalleeProvidedService.SRV_ADD_PROF_TO_SERV)) {
	    Object inWhere = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_PROF_TO_SERV_WHERE);
	    Object inWhat = call
		    .getInputValue(SCalleeProvidedService.INP_ADD_PROF_TO_SERV_WHAT);
	    if (inWhere == null || inWhat==null) {
		return ERROR_INPUT;
	    }
	    Activator.scaller.addProfToProfilable((Resource)inWhere, (Resource)inWhat);
