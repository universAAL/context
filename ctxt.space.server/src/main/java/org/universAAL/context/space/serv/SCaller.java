...
public class SCaller {
	...
	return genericGetAll(Queries.GETALL.replace(Queries.ARGTYPE,
		AALService.MY_URI), Queries.GETALLXTRA.replace(
		Queries.ARGTYPE, AALService.MY_URI));
    }

    public ArrayList getAALSpaces() {
	return genericGetAll(Queries.GETALL.replace(Queries.ARGTYPE,
		AALSpace.MY_URI), Queries.GETALLXTRA.replace(
		Queries.ARGTYPE, AALSpace.MY_URI));
    }

    protected Resource getHROfAALService(Resource input) {
	return genericGetOnePropOf(input, Queries.GETONEOFXTRA.replace(
		Queries.ARGTYPE,
		AALServiceProfile.PROP_HUMAN_RESOURCE_SUBPROFILE),
		Queries.GETONEOF.replace(Queries.ARGTYPE,
			AALServiceProfile.PROP_HUMAN_RESOURCE_SUBPROFILE));
    }
	...
	return genericGetAllOf(
		...
		Queries.GETALLOF.replace(Queries.ARG2,
			AALSpaceProfile.PROP_INSTALLED_SERVICES).replace(
			Queries.ARGTYPE, AALService.MY_URI),
	...
	return genericGetAllOf(
		input,
		Queries.GETALLOF.replace(Queries.ARG2,
			AALSpaceProfile.PROP_INSTALLED_HARDWARE).replace(
			Queries.ARGTYPE, Device.MY_URI),
		Queries.GETALLOFXTRA.replace(Queries.ARG2,
			AALSpaceProfile.PROP_INSTALLED_HARDWARE).replace(
			Queries.ARGTYPE, Device.MY_URI));
    }
	...
	return genericGetAllOf(
		input,
		Queries.GETALLOF.replace(Queries.ARG2,
			AALSpaceProfile.PROP_INSTALLED_ONTOLOGIES).replace(
			Queries.ARGTYPE, OntologyEntry.MY_URI),
		Queries.GETALLOFXTRA.replace(Queries.ARG2,
			AALSpaceProfile.PROP_INSTALLED_ONTOLOGIES).replace(
			Queries.ARGTYPE, OntologyEntry.MY_URI));
    }
	...
	return genericGetAllOf(
		input,
		Queries.GETALLOF.replace(Queries.ARG2,
			AALServiceProfile.PROP_SERVICE_OWNER).replace(
			Queries.ARGTYPE, User.MY_URI),
		Queries.GETALLOFXTRA.replace(Queries.ARG2,
			AALServiceProfile.PROP_SERVICE_OWNER).replace(
			Queries.ARGTYPE, User.MY_URI));
	...
	genericAddToSpace(input, what, Queries.ADDTOPROFILABLE.replace(
		Queries.ARGTYPE, AALSpaceProfile.PROP_INSTALLED_SERVICES));
    }
    
    protected void addDeviceToSpace(Resource input, Resource what) {
	genericAddToSpace(input, what, Queries.ADDTOPROFILABLE.replace(
		Queries.ARGTYPE, AALSpaceProfile.PROP_INSTALLED_HARDWARE));
    }
    
    protected void addOntToSpace(Resource input, Resource what) {
	genericAddToSpace(input, what, Queries.ADDTOPROFILABLE.replace(
		Queries.ARGTYPE, AALSpaceProfile.PROP_INSTALLED_ONTOLOGIES));
    }
	...
	genericAddToSpace(input, what, Queries.ADDTOPROFILABLE.replace(
		Queries.ARGTYPE, AALServiceProfile.PROP_SERVICE_OWNER));
    }
	...
	String result = getResult(defaultCaller
		.call(getDoSPARQLRequest(getquery
			.replace(Queries.ARG1, input.getURI()))));
	...
	defaultCaller
		.call(getDoSPARQLRequest(split[0]
			+ " "
			+ addquery.replace(Queries.ARGTURTLE,
				split[1])));
    }
	...
	String result2 = getResult(defaultCaller
		.call(getDoSPARQLRequest(queryallxtra)));
	...
	if (objx == null)
	    return null;
	String result = getResult(defaultCaller
		.call(getDoSPARQLRequest(queryfinal.replace(
			Queries.ARG1, input.getURI()))));
	String uri = ((Resource) objx).getURI();
	return (Resource) Activator.parser.deserialize(result, uri);
    }
