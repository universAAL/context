 ...
 * @author James Leigh
 * 
 * PATCH UAAL: Copied from NativeSailStore and modified to allow encryption and enforce cardinality 1
 */
class CardinalitySailStore implements SailStore {

	final Logger logger = LoggerFactory.getLogger(CardinalitySailStore.class);
	...
	/**
	 * Creates a new {@link CardinalitySailStore} with the default cache sizes.
	 */
	public CardinalitySailStore(File dataDir, String tripleIndexes)
	...
	/**
	 * Creates a new {@link CardinalitySailStore}.
	 */
	public CardinalitySailStore(File dataDir, String tripleIndexes, boolean forceSync, int valueCacheSize,
			...
			throws SailException
		{
			...
			try {
				...
				for (Resource context : contexts) {
					...
					if (hasMaxCardinality1(pred, contexts) && explicit) {
					    // Property has max/Card=1 & stmt not inferrd
					    removeStatements(subj, pred, null, true, contexts);
					    // rem. existing values so there is only 1
					}
