 ...
 */
public class CollectionStore extends AbstractNotifyingSail implements FederatedServiceResolverClient {
	...
	public CollectionStore(File dataDir) {
		this();
		setDataDir(dataDir);
	}

	public CollectionStore(File dataDir, String tripleIndexes, boolean encrypt) {
		this(dataDir);
		...
		throws SailException
	{
		...
		try {
			...
			final CollectionSailStore master = new CollectionSailStore(dataDir, tripleIndexes, forceSync,
			...
			this.store = new SnapshotSailStore(master, new ModelFactory() {
				...
				public Model createEmptyModel() {
					return new MemoryOverflowModel() {
						...
						{
							// Model can't fit into memory, use another CollectionSailStore to store delta
							return new CollectionSailStore(dataDir, getTripleIndexes());
						}
					};
				}
			}) {
				...
				public SailSource getExplicitSailSource() {
					if (isIsolationDisabled()) {
						// no isolation, use CollectionSailStore directly
	...
	{
		try {
			return new CollectionStoreConnection(this);
		}
		catch (IOException e) {
			throw new SailException(e);
		}
