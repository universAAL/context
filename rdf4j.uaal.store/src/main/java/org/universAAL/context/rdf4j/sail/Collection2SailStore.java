 ...
 * @author James Leigh
 * 
 * PATCH UAAL: Copied from NativeSailStore and modified to allow encryption 
 * and enforce cardinality 1 of any value on top of previous closed collections
 */
class Collection2SailStore implements SailStore {

	final Logger logger = LoggerFactory.getLogger(Collection2SailStore.class);
	...
	private IRI onProp;
	private IRI maxCard;
	private IRI exactCard;
	private IRI nonNativeInt;
	private Value one;
	...
	/**
	 * Creates a new {@link Collection2SailStore} with the default cache sizes.
	 */
	public Collection2SailStore(File dataDir, String tripleIndexes)
		throws IOException, SailException
	{
		this(dataDir, tripleIndexes, false, ValueStore.VALUE_CACHE_SIZE, ValueStore.VALUE_ID_CACHE_SIZE,
				ValueStore.NAMESPACE_CACHE_SIZE, ValueStore.NAMESPACE_ID_CACHE_SIZE, false);
	}

	/**
	 * Creates a new {@link Collection2SailStore}.
	 */
	public Collection2SailStore(File dataDir, String tripleIndexes, boolean forceSync, int valueCacheSize,
	...
	private final class NativeSailSink implements SailSink {
		...
		private boolean prevObjIsClosedCollection(Resource subj, IRI pred, Resource[] contexts) throws SailException {
			// Get all objects in S P ?
		    	try {
			CloseableIteration<? extends Statement, SailException> objs = createStatementIterator(
